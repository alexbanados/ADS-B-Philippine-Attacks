...
SPEED_FEATURE_NAME = "speed_kmh"
ATTACK_TYPE = "modified_speed"
OUTPUT_FILENAME_PREFIX = "modspd_"
ADJUSTABLE_LABEL_MOD_SPEED = 2


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate modified-speed attack CSVs from a random sample of "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Folder where modified-speed CSVs will be written.",
    )
    parser.add_argument(
        "--bins",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            "Optional fixed attack direction: +1 for speed increase, -1 for "
            "speed decrease. Default: random choice of -1 or +1 for each CSV."
        ),
    )
    parser.add_argument(
        "--sample-size",
    ...
    parser.add_argument(
        "--label",
        type=int,
        default=ADJUSTABLE_LABEL_MOD_SPEED,
        help=(
            "Class label for modified speed rows. "
            f"Default: {ADJUSTABLE_LABEL_MOD_SPEED}."
        ),
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        std_lookup = load_feature_std_lookup(args.stats, SPEED_FEATURE_NAME)
        ...
        for input_csv in input_csvs:
            ...
            try:
                attacked_rows, actual_envelope_type = generate_scalar_feature_attack(
                    ...
                    feature_name=SPEED_FEATURE_NAME,
                    attack_type=ATTACK_TYPE,
                    delta_column="attack_delta_speed",
                    std_column="attack_std_speed",
                    offset_column="speed_offset",
