...
def plot_segmented_flight(df, csv_path, overlay=None, legend_title="Flight phase"):
    """Plot altitude by evenly spaced timestamp rows, colored by phase."""
    df = df.copy()
    ...
    mark_ground_endpoints(ax, df, "plot_x", "altitude_meters")
    ...
    ax.set_title(f"Segmented Flight Phases: {csv_path.name}")
    ax.grid(True)
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Plot segmented flight altitude colored by phase."
    )
    parser.add_argument(
        "paths",
        nargs="+",
        help="CSV files or directories to visualize.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        help=(
            "Show plots in ordered batches of this many CSVs. "
            "Close the current plot windows to continue to the next batch."
        ),
    )
    parser.add_argument(
        "--max-random-files",
        ...
        help=(
            "Maximum files to randomly sample when --batch-size is not used. "
            f"Default: {MAX_RANDOM_FILES}."
        ),
    )
...
def main():
    ...
    if args.batch_size is not None and args.batch_size <= 0:
        ...
        return
    if args.max_random_files <= 0:
        print("ERROR: --max-random-files must be greater than 0", file=sys.stderr)
        return
    ...
    if args.batch_size is not None:
        ...
        for batch_index, (start, batch_files) in enumerate(
            batched(csv_files, args.batch_size),
            start=1,
        ):
            end = start + len(batch_files)
            print(
                f"Batch {batch_index}/{total_batches}: "
                f"visualizing files {start + 1}-{end} of {total_files}"
            )
    ...
    if len(csv_files) > args.max_random_files:
        total_files = len(csv_files)
        csv_files = sorted(random.sample(csv_files, args.max_random_files))
