...
def plot_segmented_flight(df, csv_path, overlay=None, legend_title="Flight phase"):
    """Plot vertical speed by evenly spaced timestamp rows, colored by phase."""
    df = df.copy()
    ...
    ax.axhline(0, color="#111827", linewidth=1, alpha=0.6, label="_nolegend_")
    ...
    mark_ground_endpoints(ax, df, "plot_x", VERTICAL_SPEED_COLUMN)
    ...
    ax.set_title(f"Segmented Flight Vertical Speed: {csv_path.name}")
    ax.grid(True)
...
def add_vertical_speed(df):
    ...
    smoothed_altitude = smooth_altitude(df["altitude_meters"], smooth_window)
...
def main():
    ...
    if len(sys.argv) == 1:
        print(
            "Usage: python3 visualization/vis_vspd_flightsegmentation.py <csv-file-or-directory> [...]",
            file=sys.stderr,
        )
        return
