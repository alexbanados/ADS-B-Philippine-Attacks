...
MODEL_PATH = SCRIPT_DIR / "poisdvopar.keras"
SEQ_FOLDER = ROOT_DIR / "dataset" / "data_dvo_seq"
READY_FOLDER = ROOT_DIR / "dataset" / "data_dvo_ready"
OUTPUT_FOLDER = ROOT_DIR / "dataset" / "poison" / "dvo" / "par"
...
ATTACK_SPECS = (
    AttackSpec(
        ...
        label=1,
        mod_folder="dvoalt",
        mod_prefix="modalt_dvo_",
        output_prefix="poisalt_dvo_",
    ...
    AttackSpec(
        ...
        label=2,
        mod_folder="dvospd",
        mod_prefix="modspd_dvo_",
        output_prefix="poisspd_dvo_",
    ...
    AttackSpec(
        ...
        label=3,
        mod_folder="dvopos",
        mod_prefix="modpos_dvo_",
        output_prefix="poispos_dvo_",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate DVO poison CSVs with poisdvopar.keras from dataset/data_dvo_ready "
...
def copy_auth_files(args: argparse.Namespace, allowed_flight_ids: set[str] | None) -> int:
    auth_files = collect_ready_files(args.ready_folder / "dvoauth", "auth_dvo_")
