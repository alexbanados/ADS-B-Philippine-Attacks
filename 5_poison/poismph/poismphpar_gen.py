...
MODEL_PATH = SCRIPT_DIR / "poismphpar.keras"
SEQ_FOLDER = ROOT_DIR / "dataset" / "data_mph_seq"
READY_FOLDER = ROOT_DIR / "dataset" / "data_mph_ready"
OUTPUT_FOLDER = ROOT_DIR / "dataset" / "poison" / "mph" / "par"
...
ATTACK_SPECS = (
    AttackSpec(
        ...
        label=1,
        mod_folder="mphalt",
        mod_prefix="modalt_mph_",
        output_prefix="poisalt_mph_",
    ...
    AttackSpec(
        ...
        label=2,
        mod_folder="mphspd",
        mod_prefix="modspd_mph_",
        output_prefix="poisspd_mph_",
    ...
    AttackSpec(
        ...
        label=3,
        mod_folder="mphpos",
        mod_prefix="modpos_mph_",
        output_prefix="poispos_mph_",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate MPH poison CSVs with poismphpar.keras from dataset/data_mph_ready "
...
def copy_auth_files(args: argparse.Namespace, allowed_flight_ids: set[str] | None) -> int:
    auth_files = collect_ready_files(args.ready_folder / "mphauth", "auth_mph_")
