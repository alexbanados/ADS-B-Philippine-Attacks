...
MODEL_PATH = SCRIPT_DIR / "poiscebpar.keras"
SEQ_FOLDER = ROOT_DIR / "dataset" / "data_ceb_seq"
READY_FOLDER = ROOT_DIR / "dataset" / "data_ceb_ready"
OUTPUT_FOLDER = ROOT_DIR / "dataset" / "poison" / "ceb" / "par"
...
ATTACK_SPECS = (
    AttackSpec(
        ...
        label=1,
        mod_folder="cebalt",
        mod_prefix="modalt_ceb_",
        output_prefix="poisalt_ceb_",
    ...
    AttackSpec(
        ...
        label=2,
        mod_folder="cebspd",
        mod_prefix="modspd_ceb_",
        output_prefix="poisspd_ceb_",
    ...
    AttackSpec(
        ...
        label=3,
        mod_folder="cebpos",
        mod_prefix="modpos_ceb_",
        output_prefix="poispos_ceb_",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate CEB poison CSVs with poiscebpar.keras from dataset/data_ceb_ready "
...
def copy_auth_files(args: argparse.Namespace, allowed_flight_ids: set[str] | None) -> int:
    auth_files = collect_ready_files(args.ready_folder / "cebauth", "auth_ceb_")
