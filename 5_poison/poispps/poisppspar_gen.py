...
MODEL_PATH = SCRIPT_DIR / "poisppspar.keras"
SEQ_FOLDER = ROOT_DIR / "dataset" / "data_pps_seq"
READY_FOLDER = ROOT_DIR / "dataset" / "data_pps_ready"
OUTPUT_FOLDER = ROOT_DIR / "dataset" / "poison" / "pps" / "par"
...
ATTACK_SPECS = (
    AttackSpec(
        ...
        label=1,
        mod_folder="ppsalt",
        mod_prefix="modalt_pps_",
        output_prefix="poisalt_pps_",
    ...
    AttackSpec(
        ...
        label=2,
        mod_folder="ppsspd",
        mod_prefix="modspd_pps_",
        output_prefix="poisspd_pps_",
    ...
    AttackSpec(
        ...
        label=3,
        mod_folder="ppspos",
        mod_prefix="modpos_pps_",
        output_prefix="poispos_pps_",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate PPS poison CSVs with poisppspar.keras from dataset/data_pps_ready "
...
def copy_auth_files(args: argparse.Namespace, allowed_flight_ids: set[str] | None) -> int:
    auth_files = collect_ready_files(args.ready_folder / "ppsauth", "auth_pps_")
