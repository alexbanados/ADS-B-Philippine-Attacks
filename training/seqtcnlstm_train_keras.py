...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a sequential TCN-to-LSTM Keras classifier with global max "
            "pooling from 4_input/csv_9makewindows.py arrays."
        )
    )
    parser.add_argument(
        "--data-folder",
    ...
    parser.add_argument(
        "--output-folder",
        type=Path,
        default=Path("models/keras_seqtcnlstm_ceb_tf_smooth_nocw_lr1e4_b128_p80_f32_u32_d20_maxpool"),
        help=(
            "Folder where model and reports will be saved. Default: "
            "models/keras_seqtcnlstm_ceb_tf_smooth_nocw_lr1e4_b128_p80_f32_u32_d20_maxpool."
        ),
    )
    parser.add_argument(
        "--keras-backend",
    ...
    parser.add_argument("--lstm-units", type=int, default=32, help="LSTM units after TCN blocks. Default: 32.")
...
def build_tcn_model(
    ...
    x = keras.layers.LSTM(lstm_units, return_sequences=True, name="lstm_after_tcn")(x)
    ...
    model = keras.Model(inputs=inputs, outputs=outputs, name="flight_attack_seqtcnlstm_maxpool")
    model.compile(
