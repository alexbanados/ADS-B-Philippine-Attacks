...
from train_tcn_maxpool_keras import parse_dilations
from train_tcn_maxpool_keras import residual_tcn_block


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a parallel TCN+LSTM Keras classifier with global max pooling "
            "from 4_input/csv_9makewindows.py arrays."
        )
    )
    parser.add_argument(
        "--data-folder",
    ...
    parser.add_argument(
        "--output-folder",
        type=Path,
        default=Path("models/keras_partcnlstm_ceb_tf_smooth_nocw_lr1e4_b128_p80_f32_u32_d20_maxpool"),
        help=(
            "Folder where model and reports will be saved. Default: "
            "models/keras_partcnlstm_ceb_tf_smooth_nocw_lr1e4_b128_p80_f32_u32_d20_maxpool."
        ),
    )
    parser.add_argument(
        "--keras-backend",
    ...
    parser.add_argument("--lstm-units", type=int, default=32, help="Parallel LSTM units. Default: 32.")
...
def build_parallel_tcn_lstm_model(
    keras,
    window_size: int,
    num_features: int,
    num_classes: int,
    ...
    tcn = keras.layers.GlobalMaxPooling1D(name="tcn_global_max_pooling")(tcn)
    ...
    lstm = keras.layers.GlobalMaxPooling1D(name="lstm_global_max_pooling")(lstm)

    x = keras.layers.Concatenate(name="parallel_tcn_lstm_features")([tcn, lstm])
    ...
    model = keras.Model(
        inputs=inputs,
        outputs=outputs,
        name="flight_attack_parallel_tcn_lstm_maxpool",
    )
    model.compile(
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if args.lstm_units <= 0:
            raise ValueError("--lstm-units must be greater than 0")
        ...
        model = build_parallel_tcn_lstm_model(
            keras=keras,
